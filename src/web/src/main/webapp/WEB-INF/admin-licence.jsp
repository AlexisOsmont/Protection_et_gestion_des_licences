...
<%@ page import="model.Licence.Status"%>
<!DOCTYPE html>
<html>
<head>
	...
	<main style="padding-left: 15%; width: 100%;">
		<section class="jumbotron text-center">
			<div class="container">
				...
				<script>
					...
					window.onload = function () {
						...
						document.getElementById('select-filter').onchange = function (event) {
						  ...
						  switch (choice) {
						  	case "<%=filtersDesc[1][0]%>":
						  		// create software filter's input
				            	label = document.createElement("label");
							    label.className = "me-2 sr-only";
							    label.textContent = "Nom du logiciel";
						  	...
						  	case "<%=filtersDesc[2][0]%>":
							    ...
							    input.type = "email";
	                  			input.id = "changeableForm-input";
	                  			input.setAttribute("name", "<%=filtersDesc[2][1]%>");
						      	break;
						    case "<%=filtersDesc[3][0]%>":
						    	// create status filter's input
						    	label = document.createElement("label");
							    label.className = "me-2 sr-only";
							    label.textContent = "Status";
	                  			label.setAttribute("for", "changeableForm-select");
	                  			
							    select = document.createElement("select");
							    select.className = "form-control";
							    select.id = "changeableForm-select";
							    select.setAttribute("name", "<%=filtersDesc[3][1]%>");
								...
								for (Status state : statusArray) {%> 
							    	...
							    	option.text = "<%=state.toString()%>";
									select.add(option);
								<%}%>
								// just to match the code below
								input = select;
								break;
							default:
								break;
						...
						switch (select.value) {
						  	...
						  	case "<%=filtersDesc[2][0]%>":
					  			// retrieve the argument in the url	
					  			arg = getParameterByName("<%=filtersDesc[2][1]%>");
					  			node.children[1].value = arg;
						      	break;
						    case "<%=filtersDesc[3][0]%>":
						    	// retrieve the argument in the url
						    	arg = getParameterByName("<%=filtersDesc[3][1]%>");
						    	let idx = Math.max(
						    		[...node.children[1].options]
						    		.map(function (x) { return x.value})
						    		.indexOf(arg),
						    		0
						    	);
						    	node.children[1].selectedIndex = idx;
								break;
							default:
								break;
						}
				...
				<form id="table-filter" style="margin-top: 5%;" class="form-inline" method="GET"
					action="/admin/licence">
					...
					<input id="page-number" type="hidden" 
						name="page" value="<%= request.getAttribute("current-page")%>">
